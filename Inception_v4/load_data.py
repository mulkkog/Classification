...
def read_test_dataset(filename, batch_size):
    ...
    dataset = dataset.map(lambda image, label: (tf.image.resize(image, size=[443, 443]), label),
    ...
    dataset = dataset.map(lambda image, label: (tf.image.random_crop(image, size=[299, 299, 3]), label),
...
def read_val_dataset(filename, batch_size):
    ...
    dataset = dataset.with_options(ignore_order)
    dataset = dataset.map(_parse_image_function, num_parallel_calls=AUTOTUNE)
    dataset = dataset.map(lambda image, label: (tf.image.resize(image, size=[299, 299]), label),
                          num_parallel_calls=tf.data.experimental.AUTOTUNE)  # resize
    dataset = dataset.batch(batch_size, drop_remainder=True)
    dataset = dataset.prefetch(buffer_size=AUTOTUNE)
    return dataset
